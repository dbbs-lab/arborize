__all__ = [
    "NeuronModel",
    "get_section_synapses",
    "get_section_receivers",
    "make_builder",
    "compose_types",
    "flatten_composite",
]
...
class NeuronModel:
    ...
    def _init_section(self, section):
        ...
        for label in section.labels:
            if label not in self.__class__.section_types:
                raise LabelNotDefinedError(
                    "Label '{}' given to a section but not defined in {}".format(
    ...
    def _apply_section_mechanisms(self, section, mechanisms):
        ...
        for mechanism, attrs in mechanisms.items():
            try:
                ...
                e = MechanismNotFoundError(
                    "Could not find '{}.{}' in the glia library".format(name, variant),
                    name,
                    variant,
                )
                raise e from None
            ...
            try:
                ...
                errr.wrap(
                    SectionAttributeError, e, prepend="No mechanisms were inserted! "
                )
    ...
    def _apply_mech_attributes(self, section, mod_name, attributes):
        ...
        for attribute, value in attributes.items():
            ...
            try:
                ...
                raise SectionAttributeError(
                    "The attribute '{}'{} is not found on a section with labels {}.".format(
                        attribute,
    ...
    def create_receiver(self, section, gid, synapse_type):
        ...
        receiver_dict = {"type": synapse_type, "synapse": synapse, "gid": gid}
        if synapse.source is not None:
            p.parallel.target_var(
                getattr(synapse._point_process, "_ref_" + synapse.source), gid
            )
    ...
    def create_synapse(self, section, synapse_type=None):
        ...
        if not hasattr(self.__class__, "synapse_types"):
            raise ModelClassError(
                "Can't connect to a NeuronModel that does not specify any `synapse_types` on its class."
            )
        synapse_types = self.__class__.synapse_types
        if (
            not hasattr(section, "available_synapse_types")
            or not section.available_synapse_types
        ):
            raise ConnectionError(
                "Can't connect to '{}' labelled section without available synapse types.".format(
                    labels_name
                )
            )
        ...
        if synapse_type is None:
            if len(section_synapses) != 1:
                raise AmbiguousSynapseError(
                    "Too many possible synapse types: "
                    + ", ".join(section_synapses)
                    + ". Specify a `synapse_type` for the connection."
                )
            else:
            ...
            if not synapse_type in section_synapses:
                raise SynapseNotPresentError(
                    "The synapse type '{}' is not present on '{}' labelled section in {}.".format(
                        synapse_type, labels_name, self.__class__.__name__
                    )
                )
            elif not synapse_type in synapse_types:
                raise SynapseNotDefinedError(
                    "The synapse type '{}' is used on '{}' labelled section but not defined in the model.".format(
                        synapse_type, labels_name
                    )
                )
            else:
                synapse_definition = synapse_types[synapse_type]
        ...
        synapse = Synapse(
            self,
            section,
            synapse_point_process,
            synapse_attributes,
            variant=synapse_variant,
            type=synapse_type,
            source=source,
        )
    ...
    def cable_cell_template(cls, morphology=0, decor=None, labels=None):
        ...
        cls._cc_insert_labels(
            labels, getattr(cls, "labels", {}), getattr(cls, "tags", {})
        )
        ...
        for label, tags in tag_map.items():
            if label not in label_dict:
                if len(tags) == 1:
                    ...
                    label_dict[
                        label
                    ] = s = f"(join {' '.join(f'(tag {tag})' for tag in tags)})"
    ...
    def _cc_insert_cable(cls, decor, label, cable):
        kwargs = {
            (p := _cc_cable_prop_map.get(c))[0]: p[1](v) for c, v in cable.items()
        }
...
def _try_mech_presence(mech, resolved):
    ...
    specifics = [
        v for m, v in resolved.items() if isinstance(m, tuple) and m[0] == mech
    ]
    if len(specifics) == 1:
        return specifics[0]
    elif len(specifics) > 1:
        raise SectionAttributeError(
            f"Section attributes were specified for `{mech}` but this could apply to: "
            + ", ".join(specifics)
        )
...
def _arb_resolve_composites(definitions, label_dict):
    ...
    for name, comp in comps.items():
        try:
            ...
            raise Exception(
                f"Unknown section type '{p}' in composite type '{name}'."
            ) from None
...
_cc_ion_prop_map = {"e": "rev_pot"}
_cc_cable_prop_map = {
    "Ra": ("rL", lambda Ra: Ra),
    "cm": ("cm", lambda cm: cm / 100),
}
...
def _import3d_builder(morphology):
    if not os.path.isfile(morphology):
        raise FileNotFoundError(
            f"'{morphology}' can't be found. Provide a correct absolute path in the `morphologies` array or add a `morphology_directory` class attribute to your NeuronModel."
        )
    with _suppress_stdout():
        ...
        try:
            ...
            raise MorphologyBuilderError(
                f"Couldn't parse '{morphology}': {e}"
            ) from None
...
def make_builder(blueprint, path=None):
    ...
    if type(blueprint) is str:
        if not os.path.isabs(blueprint):
            if path is None:
                raise MorphologyBuilderError(
                    "Morphology filestrings have to be absolute paths or a `path` keyword argument must be provided."
                )
            else:
